...
class TrueApiClient:
    ...
    async def exchange_token(self, inn: str, uuid: str, signature: str) -> str:
        ...
        `inn` (выбранная на /admin/token организация) передаётся в запрос True API:
        production подтвердил, что для реальной авторизации ЦРПТ требует ИНН
        организации (полномочия пользователя/сертификата в ГИС МТ, включая МЧД,
        определяются на стороне ЦРПТ, а не функционалом приложения).
        """
        body = {
            "uuid": uuid,
            "data": signature,
